...
function simpleExpenseEditor(item) {
...
function simpleView(budget, model) {
    const commission = expenseById(budget, "copart-commission");
    const otherCopart = expenseById(budget, "copart-other");
    const purchaseAmount = (item, label) => `<div class="kb-field"><label for="kb-simple-${item.id}">${label}</label><div class="kb-money"><input id="kb-simple-${item.id}" type="number" inputmode="decimal" min="0" step="0.01" value="${attr(numeric(item.amount))}" data-kb-expense-id="${attr(item.id)}" data-kb-expense-field="amount"><span>€</span></div></div>`;
    const groups = expenseCategories.map((category) => { const items = budget.expenses.filter((item) => item.category === category && !item.includedInPurchase); if (!items.length && category === "PERSONALIZADO")
        return ""; return `<details class="kb-expense-group"${category === "VIAJE" ? " open" : ""}><summary><span>${category}</span><strong data-kb-category="${category}">${formatEuro(model.categoryTotals[category])}</strong></summary><div class="kb-simple-expense-list">${items.map(simpleExpenseEditor).join("")}</div></details>`; }).join("");
...
function updateOutputs(root, budget) {
    ...
    root.querySelectorAll('[data-kb-field="purchase.totalRebu"]').forEach((input) => { input.placeholder = String(model.automaticPurchaseTotal); });
    root.querySelectorAll("[data-kb-purchase-help]").forEach((element) => { element.textContent = budget.purchase.totalRebu === "" ? `Automático: ${formatEuro(model.automaticPurchaseTotal)}` : "Valor manual activo. Vacíalo para volver al cálculo automático."; });
...
export function mountKairosBudgetApp(options = {}) {
    ...
    root.addEventListener("click", async (event) => {
        ...
        const budget = active();
        if (action === "view-internal" || action === "view-simple" || action === "view-client") {
            budget.view = action === "view-client" ? "client" : action === "view-simple" ? "simple" : "internal";
            persist();
            rerender();
            return;
        }
