import {
  ...
  normalizeKairosBudgetStore,
  safePdfFileName,
  type ExpenseCategory,
  type ExpenseItem,
  type KairosBudget,
  type KairosBudgetModel,
  type KairosBudgetStore,
} from "./domain.js";
...
declare global {
  interface Window { jspdf?: { jsPDF: new (options?: Record<string, unknown>) => any } }
}

const escapeHtml = (value: unknown) => String(value ?? "").replace(/[&<>"]/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[character] || character);
const attr = escapeHtml;
const numeric = (value: number | "") => value === "" ? "" : String(value);
const signedEuro = (value: number) => value > 0 ? `+${formatEuro(value)}` : formatEuro(value);
const expenseCategories: ExpenseCategory[] = ["VIAJE", "VEHÍCULO", "MATRICULACIÓN", "PERSONALIZADO"];

function loadStore(legacyState?: unknown): KairosBudgetStore {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (raw) return normalizeKairosBudgetStore(JSON.parse(raw));
  } catch (error) {
...
function saveStore(store: KairosBudgetStore) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizeKairosBudgetStore(store))); }
  catch (error) { console.warn("No se pudo guardar el presupuesto KAIROS.", error); }
}
...
function field(label: string, path: string, value: string | number, options: { type?: string; placeholder?: string; help?: string; wide?: boolean; min?: number; max?: number; step?: string } = {}) {
  const inputId = `kb-${path.replace(/[^a-z0-9]+/gi, "-")}`;
  const type = options.type || "text";
  return `<div class="kb-field${options.wide ? " kb-field--wide" : ""}"><label for="${inputId}">${escapeHtml(label)}</label><input id="${inputId}" type="${type}"${type === "number" ? ` inputmode="decimal" min="${options.min ?? 0}"${options.max !== undefined ? ` max="${options.max}"` : ""} step="${options.step || "0.01"}"` : ""} value="${attr(value)}" placeholder="${attr(options.placeholder || "")}" data-kb-field="${attr(path)}">${options.help ? `<small${path === "purchase.totalRebu" ? " data-kb-purchase-help" : ""}>${escapeHtml(options.help)}</small>` : ""}</div>`;
}

function selectField(label: string, path: string, value: string, options: string[], labels: Record<string, string> = {}) {
  const inputId = `kb-${path.replace(/[^a-z0-9]+/gi, "-")}`;
...
function vehicleFields(budget: KairosBudget) {
...
function purchaseSection(budget: KairosBudget, model: KairosBudgetModel) {
  const commission = expenseById(budget, "copart-commission")!;
  const other = expenseById(budget, "copart-other")!;
  const purchaseExpense = (item: ExpenseItem, label: string) => `<div class="kb-field"><label for="kb-${item.id}">${label}</label><div class="kb-money"><input id="kb-${item.id}" type="number" inputmode="decimal" min="0" step="0.01" value="${attr(numeric(item.amount))}" data-kb-expense-id="${item.id}" data-kb-expense-field="amount"><span>€</span></div></div>`;
...
function expenseEditor(item: ExpenseItem) {
...
function expensesSection(budget: KairosBudget, model: KairosBudgetModel) {
  return `<section class="kb-card" aria-labelledby="kb-expenses-title"><div class="kb-section-head"><div><span>03</span><h2 id="kb-expenses-title">Gastos de la operación</h2></div><button type="button" class="kb-button kb-button--secondary" data-kb-action="add-expense">+ Añadir gasto personalizado</button></div>
    <div class="kb-expense-groups">${expenseCategories.map((category) => { const items = budget.expenses.filter((item) => item.category === category && !item.includedInPurchase); if (!items.length && category === "PERSONALIZADO") return ""; return `<details class="kb-expense-group"${category === "VIAJE" ? " open" : ""}><summary><span>${category}</span><strong data-kb-category="${category}">${formatEuro(model.categoryTotals[category])}</strong></summary><div class="kb-expense-grid">${items.map(expenseEditor).join("")}</div></details>`; }).join("")}</div>
...
function simpleExpenseEditor(item: ExpenseItem) {
  return `<div class="kb-simple-expense" data-kb-expense-card="${attr(item.id)}"><div class="kb-simple-expense-name">${item.custom ? `<input aria-label="Nombre del gasto" value="${attr(item.name)}" data-kb-expense-id="${attr(item.id)}" data-kb-expense-field="name">` : `<span>${escapeHtml(item.name)}</span>`}${item.custom ? `<button type="button" class="kb-icon-button" data-kb-action="remove-expense" data-kb-id="${attr(item.id)}" aria-label="Eliminar ${attr(item.name)}">×</button>` : ""}</div><div class="kb-money"><input aria-label="Importe de ${attr(item.name)}" type="number" inputmode="decimal" min="0" step="0.01" value="${attr(numeric(item.amount))}" data-kb-expense-id="${attr(item.id)}" data-kb-expense-field="amount"><span>€</span></div></div>`;
}

function simpleView(budget: KairosBudget, model: KairosBudgetModel) {
  const commission = expenseById(budget, "copart-commission")!;
  const otherCopart = expenseById(budget, "copart-other")!;
  const purchaseAmount = (item: ExpenseItem, label: string) => `<div class="kb-field"><label for="kb-simple-${item.id}">${label}</label><div class="kb-money"><input id="kb-simple-${item.id}" type="number" inputmode="decimal" min="0" step="0.01" value="${attr(numeric(item.amount))}" data-kb-expense-id="${attr(item.id)}" data-kb-expense-field="amount"><span>€</span></div></div>`;
  const groups = expenseCategories.map((category) => { const items = budget.expenses.filter((item) => item.category === category && !item.includedInPurchase); if (!items.length && category === "PERSONALIZADO") return ""; return `<details class="kb-expense-group"${category === "VIAJE" ? " open" : ""}><summary><span>${category}</span><strong data-kb-category="${category}">${formatEuro(model.categoryTotals[category])}</strong></summary><div class="kb-simple-expense-list">${items.map(simpleExpenseEditor).join("")}</div></details>`; }).join("");
  return `<div class="kb-simple-workspace" data-kb-view-panel="simple"><section class="kb-card"><div class="kb-section-head"><div><span>01</span><h2>Compra del vehículo</h2></div><p>Introduce el precio y los gastos. No necesitas completar mercado, beneficio ni cálculos comerciales.</p></div><div class="kb-form-grid kb-form-grid--purchase">${field("Precio del vehículo / puja", "purchase.hammer", numeric(budget.purchase.hammer), { type: "number", placeholder: "14000" })}${purchaseAmount(commission, "Comisión Copart")}${purchaseAmount(otherCopart, "Otros gastos Copart")}${field("Precio total de compra", "purchase.totalRebu", numeric(budget.purchase.totalRebu), { type: "number", placeholder: String(model.automaticPurchaseTotal), help: budget.purchase.totalRebu === "" ? `Automático: ${formatEuro(model.automaticPurchaseTotal)}` : "Valor manual activo. Vacíalo para sumar precio, comisión y otros gastos Copart." })}</div><div class="kb-number-strip"><div><span>Compra del vehículo</span><strong data-kb-output="purchaseRebu">${formatEuro(model.purchaseRebu)}</strong></div><div><span>Otros gastos</span><strong data-kb-output="operatingExpensesGross">${formatEuro(model.operatingExpensesGross)}</strong></div><div class="is-primary"><span>Coste total estimado</span><strong data-kb-output="totalPaid">${formatEuro(model.totalPaid)}</strong></div></div></section><section class="kb-card"><div class="kb-section-head"><div><span>02</span><h2>Gastos</h2></div><button type="button" class="kb-button kb-button--secondary" data-kb-action="add-expense">+ Añadir gasto</button></div><p class="kb-note">Aquí solo introduces importes pagados. Para detallar IVA, país o notas utiliza la Vista interna.</p><div class="kb-expense-groups">${groups}</div><div class="kb-simple-total"><span>Coste total estimado</span><strong data-kb-output="totalPaid">${formatEuro(model.totalPaid)}</strong><small>Compra del vehículo + todos los gastos introducidos.</small></div></section></div>`;
}

function pricingSection(budget: KairosBudget, model: KairosBudgetModel) {
...
function inverseSection(model: KairosBudgetModel) {
...
function alertMarkup(model: KairosBudgetModel) {
...
function internalView(budget: KairosBudget, model: KairosBudgetModel) {
...
function includedItems(detail: "simple" | "detailed") {
...
function clientView(budget: KairosBudget, model: KairosBudgetModel) {
...
function topCards(budget: KairosBudget, model: KairosBudgetModel) {
  if (budget.view === "client") return `<div class="kb-top-cards kb-top-cards--client"><div><span>Mercado España</span><strong data-kb-output="marketSpain">${formatEuro(model.marketSpain)}</strong></div><div><span>Precio final</span><strong data-kb-output="finalPrice">${formatEuro(model.finalPrice)}</strong></div><div data-kb-signal="${model.signal}"><span>Ahorro cliente</span><strong data-kb-output="clientSavings">${signedEuro(model.clientSavings)}</strong></div><div><span>Reserva</span><strong data-kb-output="reservation">${formatEuro(model.reservation)}</strong></div></div>`;
  if (budget.view === "simple") return `<div class="kb-top-cards kb-top-cards--simple"><div><span>Compra del vehículo</span><strong data-kb-output="purchaseRebu">${formatEuro(model.purchaseRebu)}</strong></div><div><span>Otros gastos</span><strong data-kb-output="operatingExpensesGross">${formatEuro(model.operatingExpensesGross)}</strong></div><div class="is-private"><span>Coste total estimado</span><strong data-kb-output="totalPaid">${formatEuro(model.totalPaid)}</strong></div></div>`;
...
function renderWorkspace(store: KairosBudgetStore) {
  const budget = currentKairosBudget(store);
  const model = calculateKairosBudget(budget);
  const description = budget.view === "internal" ? "Todos los cálculos reales. No compartas capturas de esta vista." : budget.view === "simple" ? "Introduce compra y gastos para obtener el total, sin completar el presupuesto avanzado." : "Solo información comercial preparada para compartir.";
  const content = budget.view === "internal" ? internalView(budget, model) : budget.view === "simple" ? simpleView(budget, model) : clientView(budget, model);
  return `<section class="kairos-budget" data-kairos-budget data-view="${budget.view}"><div class="kb-toolbar"><div class="kb-budget-picker"><label for="kb-budget-select">Presupuesto activo</label><select id="kb-budget-select" data-kb-select>${store.budgets.map((item) => `<option value="${attr(item.id)}"${item.id === budget.id ? " selected" : ""}>${escapeHtml(budgetDisplayName(item))} · ${escapeHtml(item.status)}</option>`).join("")}</select></div><div class="kb-toolbar-actions"><button class="kb-button kb-button--secondary" type="button" data-kb-action="new"${store.budgets.length >= MAX_BUDGETS ? " disabled" : ""}>Nuevo</button><button class="kb-button kb-button--secondary" type="button" data-kb-action="duplicate"${store.budgets.length >= MAX_BUDGETS ? " disabled" : ""}>Duplicar</button><button class="kb-button kb-button--secondary" type="button" data-kb-action="save">Guardar</button><button class="kb-button kb-button--danger" type="button" data-kb-action="delete">Eliminar</button></div><div class="kb-status-field"><label for="kb-status">Estado</label><select id="kb-status" data-kb-field="status">${STATUS_OPTIONS.map((status) => `<option${status === budget.status ? " selected" : ""}>${status}</option>`).join("")}</select></div></div><div class="kb-view-tabs" role="tablist" aria-label="Vista del presupuesto"><button type="button" role="tab" aria-selected="${budget.view === "internal"}" data-kb-action="view-internal">Vista interna</button><button type="button" role="tab" aria-selected="${budget.view === "simple"}" data-kb-action="view-simple">Solo gastos</button><button type="button" role="tab" aria-selected="${budget.view === "client"}" data-kb-action="view-client">Vista cliente</button><p>${description}</p></div>${topCards(budget, model)}${content}<div class="kb-toast" data-kb-toast role="status" aria-live="polite"></div></section>`;
}

export function renderKairosBudgetApp(legacyState?: unknown): string {
  return renderWorkspace(loadStore(legacyState));
}

function setPath(target: Record<string, any>, path: string, rawValue: unknown) {
  const parts = path.split(".");
  const key = parts.pop()!;
  let object = target;
  parts.forEach((part) => { object = object[part] ||= {}; });
  const current = object[key];
  if (typeof current === "boolean") object[key] = Boolean(rawValue);
  else if (NUMERIC_PATHS.has(path) || typeof current === "number") {
    if (rawValue === "") object[key] = "";
    else { const value = Number(rawValue); object[key] = Number.isFinite(value) && value >= 0 ? value : ""; }
  } else object[key] = String(rawValue ?? "").slice(0, key === "notes" ? 1000 : 500);
}

function updateOutputs(root: HTMLElement, budget: KairosBudget) {
  const model = calculateKairosBudget(budget);
  const currencyOutputs: Record<string, number> = { hammer: model.hammer, copartCommission: model.copartCommission, purchaseRebu: model.purchaseRebu, operatingExpensesGross: model.operatingExpensesGross, totalPaid: model.totalPaid, totalNetCost: model.totalNetCost, spanishRecoverableVat: model.spanishRecoverableVat, foreignRecoverableVat: model.foreignRecoverableVat, recoverableVat: model.recoverableVat, netExpenses: model.netExpenses, buffer: model.buffer, requiredSalePrice: model.requiredSalePrice, finalPrice: model.finalPrice, marginRebu: model.marginRebu, vatRebu: model.vatRebu, netMarginRebu: model.netMarginRebu, realProfit: model.realProfit, maximumPurchase: model.maximumPurchase, maximumHammer: model.maximumHammer, reservation: model.reservation, marketSpain: model.marketSpain, recommendedHammer: model.hammer || Math.max(0, model.maximumHammer) };
  Object.entries(currencyOutputs).forEach(([name, value]) => root.querySelectorAll<HTMLElement>(`[data-kb-output="${name}"]`).forEach((element) => { element.textContent = formatEuro(value); }));
  root.querySelectorAll<HTMLElement>('[data-kb-output="clientSavings"]').forEach((element) => { element.textContent = signedEuro(model.clientSavings); });
  const percentageOutputs: Record<string, number> = { clientSavingsPercent: model.clientSavingsPercent, saleMarginPercent: model.saleMarginPercent, roiPercent: model.roiPercent, profitOnPurchasePercent: model.profitOnPurchasePercent };
  Object.entries(percentageOutputs).forEach(([name, value]) => root.querySelectorAll<HTMLElement>(`[data-kb-output="${name}"]`).forEach((element) => { element.textContent = formatPercent(value); }));
  Object.entries(model.categoryTotals).forEach(([category, value]) => { const output = root.querySelector<HTMLElement>(`[data-kb-category="${category}"]`); if (output) output.textContent = formatEuro(value); });
  root.querySelectorAll<HTMLInputElement>('[data-kb-field="purchase.totalRebu"]').forEach((input) => { input.placeholder = String(model.automaticPurchaseTotal); });
  root.querySelectorAll<HTMLElement>("[data-kb-purchase-help]").forEach((element) => { element.textContent = budget.purchase.totalRebu === "" ? `Automático: ${formatEuro(model.automaticPurchaseTotal)}` : "Valor manual activo. Vacíalo para volver al cálculo automático."; });
  const alerts = root.querySelector<HTMLElement>("[data-kb-alerts]"); if (alerts) alerts.innerHTML = alertMarkup(model);
  root.querySelectorAll<HTMLElement>("[data-kb-signal]").forEach((element) => { element.dataset.kbSignal = model.signal; });
  return model;
}

let pdfPromise: Promise<any> | undefined;
...
function pdfFrame(JsPDF: any, title: string, subtitle: string) {
  const doc = new JsPDF({ unit: "mm", format: "a4", compress: true });
  const width = doc.internal.pageSize.getWidth();
  const height = doc.internal.pageSize.getHeight();
  const margin = 16;
  let y = 17;
  const navy = [13, 37, 62]; const blue = [22, 101, 202]; const muted = [89, 107, 125]; const pale = [241, 246, 251];
  const ensure = (space: number) => { if (y + space > height - 16) { doc.addPage(); y = 17; } };
  const textLine = (label: string, value: string, emphasis = false) => { ensure(8); if (Math.round(y) % 2 === 0) { doc.setFillColor(...pale); doc.rect(margin, y - 4.8, width - margin * 2, 7, "F"); } doc.setTextColor(...navy); doc.setFont("helvetica", emphasis ? "bold" : "normal"); doc.setFontSize(8.5); doc.text(label, margin + 2, y); doc.text(value, width - margin - 2, y, { align: "right" }); y += 7; };
  const section = (label: string) => { ensure(13); y += 4; doc.setFont("helvetica", "bold"); doc.setFontSize(12); doc.setTextColor(...blue); doc.text(label, margin, y); y += 6; };
  const paragraph = (value: string, size = 8.5) => { const lines = doc.splitTextToSize(value, width - margin * 2); ensure(lines.length * 4.2 + 3); doc.setFont("helvetica", "normal"); doc.setFontSize(size); doc.setTextColor(...muted); doc.text(lines, margin, y); y += lines.length * 4.2 + 2; };
  doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.setTextColor(...blue); doc.text("KAIROS", margin, y); y += 9; doc.setFontSize(22); doc.setTextColor(...navy); doc.text(doc.splitTextToSize(title, width - margin * 2), margin, y); y += 10; doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(...muted); doc.text(subtitle, margin, y); y += 7; doc.setDrawColor(...blue); doc.line(margin, y, width - margin, y); y += 5;
  const finish = () => { const pages = doc.getNumberOfPages(); for (let page = 1; page <= pages; page += 1) { doc.setPage(page); doc.setFontSize(7); doc.setTextColor(...muted); doc.text(`KAIROS · ${page}/${pages}`, width - margin, height - 8, { align: "right" }); } return doc; };
  return { doc, width, margin, get y() { return y; }, set y(value: number) { y = value; }, ensure, textLine, section, paragraph, finish, colors: { navy, blue, muted, pale } };
}

function buildClientPdf(JsPDF: any, budget: KairosBudget) {
  ...
  const { doc } = pdf;
  if (safe.vehicle.photoDataUrl) { try { const imageFormat = safe.vehicle.photoDataUrl.startsWith("data:image/png") ? "PNG" : safe.vehicle.photoDataUrl.startsWith("data:image/webp") ? "WEBP" : "JPEG"; pdf.ensure(48); doc.addImage(safe.vehicle.photoDataUrl, imageFormat, pdf.margin, pdf.y, 72, 42, undefined, "FAST"); pdf.y += 47; } catch { /* La imagen es opcional. */ } }
  pdf.section("Vehículo");
  if (safe.vehicle.taxRegime) pdf.textLine("Régimen fiscal", safe.vehicle.taxRegime.toUpperCase(), true);
  ...
  if (safe.mode === "estimated") { const min = safe.estimatedMin || safe.finalPrice; const max = safe.estimatedMax || safe.finalPrice; pdf.textLine("Precio estimado matriculado", `${formatEuro(Math.min(min, max))} - ${formatEuro(Math.max(min, max))}`, true); }
  else pdf.textLine("Precio máximo llave en mano", formatEuro(safe.finalPrice), true);
  ...
  pdf.section("Reserva para activar la operación"); pdf.textLine("Reserva necesaria", formatEuro(safe.reservation), true); pdf.paragraph("Para activar la operación se solicita una reserva del 10% de la puja máxima, con un mínimo de 500 €. Esta cantidad forma parte del precio final de la operación y no supone un coste adicional.");
  const created = new Date(); const validUntil = new Date(created); validUntil.setDate(validUntil.getDate() + safe.validityDays);
  pdf.section("Condiciones"); pdf.textLine("Fecha", new Intl.DateTimeFormat("es-ES").format(created)); pdf.textLine("Válido hasta", new Intl.DateTimeFormat("es-ES").format(validUntil)); pdf.paragraph(safe.mode === "closed" ? "El precio incluye la operación completa hasta entregar el vehículo matriculado en España, salvo circunstancias extraordinarias previamente comunicadas." : "Estimación inicial sujeta a adjudicación, documentación y comprobaciones de la operación.");
  return pdf.finish();
}

function buildInternalPdf(JsPDF: any, budget: KairosBudget) {
  const model = calculateKairosBudget(budget); const title = [budget.vehicle.make, budget.vehicle.model, budget.vehicle.year].filter(Boolean).join(" ") || "Informe interno"; const pdf = pdfFrame(JsPDF, title, "Informe interno de operación · confidencial");
  if (budget.vehicle.taxRegime !== "No consta") { pdf.section("Vehículo"); pdf.textLine("Régimen fiscal", budget.vehicle.taxRegime.toUpperCase(), true); }
  pdf.section("Compra Copart"); pdf.textLine("Puja / martillo", formatEuro(model.hammer)); pdf.textLine("Comisión Copart", formatEuro(model.copartCommission)); pdf.textLine("Otros gastos Copart", formatEuro(model.copartOther)); pdf.textLine("Compra total REBU", formatEuro(model.purchaseRebu), true);
  pdf.section("Gastos"); budget.expenses.filter((item) => !item.includedInPurchase && Number(item.amount) > 0).forEach((item) => pdf.textLine(`${item.category} · ${item.name}`, formatEuro(Number(item.amount)))); pdf.textLine("Gastos brutos", formatEuro(model.operatingExpensesGross)); pdf.textLine("IVA español deducible", formatEuro(model.spanishRecoverableVat)); pdf.textLine("IVA extranjero aplicado", formatEuro(budget.recovery.includeForeignVat ? model.foreignRecoverableVat : 0)); pdf.textLine("Gastos netos", formatEuro(model.netExpenses), true);
  pdf.section("REBU y rentabilidad"); pdf.textLine("Precio final cliente", formatEuro(model.finalPrice)); pdf.textLine("Margen REBU", formatEuro(model.marginRebu)); pdf.textLine(`IVA REBU (${formatPercent(model.vatRate)})`, formatEuro(model.vatRebu)); pdf.textLine("Margen después de IVA", formatEuro(model.netMarginRebu)); pdf.textLine("Beneficio objetivo", formatEuro(model.targetProfit)); pdf.textLine("Colchón", formatEuro(model.buffer)); pdf.textLine("Beneficio real antes de Sociedades", formatEuro(model.realProfit), true); pdf.textLine("Margen sobre venta", formatPercent(model.saleMarginPercent)); pdf.textLine("ROI sobre invertido", formatPercent(model.roiPercent)); pdf.textLine("Beneficio sobre compra", formatPercent(model.profitOnPurchasePercent));
  pdf.section("Calculadora inversa"); pdf.textLine("Precio máximo compra Copart", formatEuro(model.maximumPurchase)); pdf.textLine("Puja máxima de martillo", formatEuro(model.maximumHammer), true); pdf.section("Alertas"); model.alerts.forEach((alert) => pdf.paragraph(`${alert.level === "danger" ? "NO" : "ATENCIÓN"}: ${alert.text}`)); if (!model.alerts.length) pdf.paragraph("Sin alertas con los umbrales configurados."); pdf.section("Notas"); pdf.paragraph(budget.vehicle.notes || "Sin notas internas.");
  return pdf.finish();
}

function toast(root: HTMLElement, message: string, error = false) {
  const element = root.querySelector<HTMLElement>("[data-kb-toast]"); if (!element) return; element.textContent = message; element.dataset.error = String(error); element.classList.add("is-visible"); window.setTimeout(() => element.classList.remove("is-visible"), 3500);
}

export function mountKairosBudgetApp(options: { root?: HTMLElement | null; legacyState?: unknown } = {}) {
  ensureStyle();
  const root = options.root || document.querySelector<HTMLElement>("[data-kairos-budget]");
  if (!root) return;
  ...
  root.addEventListener("input", (event) => {
    ...
    const budget = active();
    if (target.dataset.kbField) { const value = target instanceof HTMLInputElement && target.type === "checkbox" ? target.checked : target.value; setPath(budget as any, target.dataset.kbField, value); persist(); updateOutputs(root, budget); }
    if (target.dataset.kbExpenseId && target.dataset.kbExpenseField) { const item = expenseById(budget, target.dataset.kbExpenseId); if (!item) return; const key = target.dataset.kbExpenseField as keyof ExpenseItem; const value = target instanceof HTMLInputElement && target.type === "checkbox" ? target.checked : target.value; if (["amount", "deductibleVat"].includes(String(key))) (item as any)[key] = value === "" ? "" : Math.max(0, Number(value) || 0); else if (key === "vatIncluded") item.vatIncluded = Boolean(value); else (item as any)[key] = String(value).slice(0, key === "note" ? 300 : 120); persist(); updateOutputs(root, budget); }
  });
  root.addEventListener("change", (event) => {
    const target = event.target as HTMLInputElement | HTMLSelectElement;
    if (target.matches("[data-kb-select]")) { store.currentId = target.value; saveStore(store); rerender(); return; }
    if (target.matches("[data-kb-photo]")) { const file = (target as HTMLInputElement).files?.[0]; if (!file) return; if (!/^image\/(png|jpeg|webp)$/i.test(file.type) || file.size > 2_000_000) { toast(root, "La foto debe ser JPG, PNG o WebP y ocupar menos de 2 MB.", true); target.value = ""; return; } const reader = new FileReader(); reader.addEventListener("load", () => { active().vehicle.photoDataUrl = String(reader.result || ""); persist(); rerender(); toast(root, "Foto guardada en este dispositivo."); }, { once: true }); reader.readAsDataURL(file); return; }
    if (target.dataset.kbField && ["view", "client.mode", "client.detail", "status"].includes(target.dataset.kbField)) rerender();
    if (target.dataset.kbExpenseField === "vatIncluded") updateOutputs(root, active());
  });
  root.addEventListener("click", async (event) => {
    const button = (event.target as HTMLElement).closest<HTMLButtonElement>("[data-kb-action]"); if (!button) return; const action = button.dataset.kbAction; const budget = active();
    if (action === "view-internal" || action === "view-simple" || action === "view-client") { budget.view = action === "view-client" ? "client" : action === "view-simple" ? "simple" : "internal"; persist(); rerender(); return; }
    if (action === "save") { persist(); rerender(); toast(root, "Presupuesto guardado en este dispositivo."); return; }
    if (action === "new") { if (store.budgets.length >= MAX_BUDGETS) { toast(root, `Puedes guardar hasta ${MAX_BUDGETS} presupuestos.`, true); return; } const next = createEmptyKairosBudget(); store = { ...store, currentId: next.id, budgets: [...store.budgets, next] }; saveStore(store); rerender(); return; }
    if (action === "duplicate") { store = duplicateKairosBudget(store); saveStore(store); rerender(); toast(root, "Presupuesto duplicado."); return; }
    if (action === "delete") { if (!window.confirm("¿Eliminar este presupuesto? Esta acción no se puede deshacer.")) return; store.budgets = store.budgets.filter((item) => item.id !== budget.id); if (!store.budgets.length) store = createKairosBudgetStore(); else store.currentId = store.budgets[0].id; saveStore(store); rerender(); return; }
    if (action === "add-expense") { const next = addCustomExpense(budget); store.budgets = store.budgets.map((item) => item.id === budget.id ? next : item); saveStore(store); rerender(); window.requestAnimationFrame(() => root.querySelector<HTMLInputElement>("[data-kb-expense-card]:last-of-type input")?.focus()); return; }
    if (action === "remove-expense") { const item = expenseById(budget, button.dataset.kbId || ""); if (!item?.custom || !window.confirm(`¿Eliminar el gasto «${item.name}»?`)) return; budget.expenses = budget.expenses.filter((candidate) => candidate.id !== item.id); persist(); rerender(); return; }
    if (action === "remove-photo") { budget.vehicle.photoDataUrl = ""; persist(); rerender(); return; }
    if (action === "use-required-price") { budget.pricing.finalPrice = calculateKairosBudget(budget).requiredSalePrice; persist(); rerender(); return; }
    if (action === "pdf-client" || action === "pdf-internal") { if (button.disabled) return; const original = button.textContent || "Generar PDF"; button.disabled = true; button.setAttribute("aria-busy", "true"); button.textContent = "Generando PDF…"; try { const JsPDF = await loadPdfLibrary(); const kind = action === "pdf-client" ? "Cliente" : "Interno"; const doc = action === "pdf-client" ? buildClientPdf(JsPDF, budget) : buildInternalPdf(JsPDF, budget); doc.save(safePdfFileName(budget, kind)); toast(root, `PDF ${kind.toLowerCase()} descargado.`); } catch (error) { console.error("No se pudo generar el PDF KAIROS", error); toast(root, "No hemos podido generar el PDF. Inténtalo de nuevo.", true); } finally { button.disabled = false; button.removeAttribute("aria-busy"); button.textContent = original; } }
  });
}

export function kairosBudgetHasData(legacyState?: unknown): boolean {
  const store = loadStore(legacyState); const budget = currentKairosBudget(store); const model = calculateKairosBudget(budget);
...
export function setKairosBudgetPurchase(value: number, legacyState?: unknown) {
