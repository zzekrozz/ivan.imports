...
export type ExpenseCategory = "SUBASTA" | "VIAJE" | "VEHÍCULO" | "MATRICULACIÓN" | "PERSONALIZADO";
export type TaxRegime = "No consta" | "Sin REBU" | "Con REBU";
...
export interface ExpenseItem {
  ...
  country: string;
  note: string;
  includedInPurchase?: boolean;
  custom?: boolean;
}

export interface KairosBudget {
  id: string;
  createdAt: string;
  updatedAt: string;
  status: BudgetStatus;
  ...
  recovery: { includeForeignVat: boolean };
  pricing: {
    vatRate: number;
    targetProfit: number | "";
    buffer: number | "";
  ...
  client: {
    mode: ClientBudgetMode;
    estimatedMin: number | "";
    estimatedMax: number | "";
    detail: ClientDetail;
    validityDays: number;
  };
  alerts: { savingsThreshold: number; expenseRatioThreshold: number };
}

export interface KairosBudgetStore {
  version: number;
  currentId: string;
  budgets: KairosBudget[];
}
...
const expense = (expenseId: string, category: ExpenseCategory, name: string, includedInPurchase = false): ExpenseItem => ({
  id: expenseId,
  category,
  name,
  amount: "",
...
export const REQUIRED_EXPENSES: ReadonlyArray<Readonly<ExpenseItem>> = Object.freeze([
...
const emptyVehicle = (): VehicleData => ({ make: "", model: "", year: "", mileage: "", fuel: "", transmission: "", color: "", taxRegime: "No consta", copartLocation: "", listingUrl: "", marketSpain: "", notes: "", photoDataUrl: "" });

export function createEmptyKairosBudget(): KairosBudget {
  const createdAt = now();
  return {
...
function normalizeExpense(value: Partial<ExpenseItem>, fallback?: Readonly<ExpenseItem>): ExpenseItem {
  ...
  const category = (["SUBASTA", "VIAJE", "VEHÍCULO", "MATRICULACIÓN", "PERSONALIZADO"].includes(String(value.category)) ? value.category : fallback?.category || "PERSONALIZADO") as ExpenseCategory;
  return {
...
export function normalizeKairosBudget(value: unknown): KairosBudget {
  const base = createEmptyKairosBudget();
  if (!value || typeof value !== "object") return base;
  const source = value as Partial<KairosBudget>;
  ...
  const statuses: BudgetStatus[] = ["BORRADOR", "ENVIADO", "CLIENTE INTERESADO", "RESERVADO", "PUJANDO", "ADJUDICADO", "PERDIDO", "FINALIZADO"];
...
export function createKairosBudgetStore(budget = createEmptyKairosBudget()): KairosBudgetStore {
...
export function normalizeKairosBudgetStore(value: unknown): KairosBudgetStore {
  if (!value || typeof value !== "object") return createKairosBudgetStore();
  const source = value as Partial<KairosBudgetStore>;
  const budgets = Array.isArray(source.budgets) ? source.budgets.slice(0, MAX_BUDGETS).map(normalizeKairosBudget) : [];
  if (!budgets.length) return createKairosBudgetStore();
...
export function currentKairosBudget(store: KairosBudgetStore): KairosBudget {
  return store.budgets.find((budget) => budget.id === store.currentId) || store.budgets[0];
}

export function expenseById(budget: KairosBudget, expenseId: string): ExpenseItem | undefined {
  return budget.expenses.find((item) => item.id === expenseId);
}

export interface KairosBudgetModel {
  budget: KairosBudget;
  hammer: number;
  copartCommission: number;
  copartOther: number;
  ...
  roiPercent: number;
  profitOnPurchasePercent: number;
...
export function calculateKairosBudget(value: unknown): KairosBudgetModel {
  ...
  const recoverableFor = (item: ExpenseItem) => item.vatIncluded ? Math.min(finite(item.deductibleVat), finite(item.amount)) : 0;
  const isSpain = (country: string) => /^(españa|espana|spain|es)$/i.test(country.trim());
  ...
  const categoryTotals = Object.fromEntries((["SUBASTA", "VIAJE", "VEHÍCULO", "MATRICULACIÓN", "PERSONALIZADO"] as ExpenseCategory[]).map((category) => [category, roundMoney(budget.expenses.filter((item) => item.category === category && !item.includedInPurchase).reduce((total, item) => total + finite(item.amount), 0))])) as Record<ExpenseCategory, number>;
  const alerts: KairosBudgetModel["alerts"] = [];
  if (!purchaseRebu) alerts.push({ level: "warning", text: "Falta el precio de compra REBU." });
  if (!finalPrice) alerts.push({ level: "warning", text: "Falta el precio final del cliente." });
  if (finalPrice > 0 && marginRebu > 0 && vatRate === 0) alerts.push({ level: "warning", text: "El IVA REBU no está calculado porque el tipo configurado es 0 %." });
  if (finalPrice > 0 && marginRebu <= 0) alerts.push({ level: "danger", text: "El precio de venta no cubre la compra REBU." });
  if (finalPrice > 0 && realProfit < 0) alerts.push({ level: "danger", text: "El precio de venta no cubre la compra, los gastos, el IVA REBU y el colchón previstos." });
  if (targetProfit > 0 && realProfit < targetProfit) alerts.push({ level: "warning", text: `Esta operación dejaría ${formatEuro(realProfit)} frente a los ${formatEuro(targetProfit)} de beneficio objetivo.` });
  if (marketSpain > 0 && finalPrice > marketSpain) alerts.push({ level: "danger", text: "El precio final supera el valor comparable indicado para España." });
  if (marketSpain > 0 && clientSavingsPercent < budget.alerts.savingsThreshold) alerts.push({ level: "warning", text: `El ahorro para el cliente está por debajo del ${formatPercent(budget.alerts.savingsThreshold)} configurado.` });
  if (finalPrice > 0 && expenseRatioPercent > budget.alerts.expenseRatioThreshold) alerts.push({ level: "warning", text: `Los gastos netos superan el ${formatPercent(budget.alerts.expenseRatioThreshold)} del precio final.` });
...
export function duplicateKairosBudget(store: KairosBudgetStore): KairosBudgetStore {
  if (store.budgets.length >= MAX_BUDGETS) return store;
...
export function migrateLegacyCostCalculator(value: unknown): KairosBudget | null {
  if (!value || typeof value !== "object") return null;
  const legacy = value as Record<string, any>;
  if (![2, 3, 4].includes(Number(legacy.version || legacy.schemaVersion))) return null;
  const budget = createEmptyKairosBudget();
  const vehicle = legacy.vehicle || {};
  budget.vehicle = { ...budget.vehicle, make: text(vehicle.make, 80), model: text(vehicle.model, 100), year: text(vehicle.year, 10), mileage: text(vehicle.mileage, 20), fuel: text(vehicle.fuel, 50), transmission: text(vehicle.transmission, 50), color: text(vehicle.color, 50), taxRegime: (["Sin REBU", "Con REBU"].includes(String(vehicle.taxRegime)) ? vehicle.taxRegime : "No consta") as TaxRegime, listingUrl: text(vehicle.listingUrl, 500), notes: text(vehicle.notes, 1000), marketSpain: finite(legacy.marketScenarios?.[0]?.priceSpain) || "" };
  ...
  const mapping: Record<string, string> = { fees: "vehicle-other", flight: "travel-flight", localTransport: "travel-taxi", exportPlates: "vehicle-plates", temporaryInsurance: "vehicle-insurance", fuelCost: "travel-fuel-de", tolls: "travel-tolls", hotel: "travel-hotel", food: "travel-food", travelOther: "travel-other", itv: "registration-itv", coc: "registration-coc", dgt: "registration-dgt", ivtm: "registration-ivtm", iedmt: "registration-576", registrationPlates: "registration-plates", agency: "registration-agency", administrationOther: "registration-other", maintenance: "vehicle-other", repairs: "vehicle-repairs", detailing: "vehicle-cleaning", vehicleTransport: "vehicle-transport", other: "vehicle-other" };
  Object.entries(mapping).forEach(([legacyId, expenseId]) => { const target = expenseById(budget, expenseId); const amount = finite(legacy.expenses?.[legacyId]); if (target && amount) target.amount = roundMoney(finite(target.amount) + amount); });
...
export function budgetDisplayName(budget: KairosBudget): string {
...
export function safePdfFileName(budget: KairosBudget, kind: "Cliente" | "Interno"): string {
...
export function clientPdfModel(value: unknown) {
